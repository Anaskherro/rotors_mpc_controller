...
class PositionNMPC:
    ...
    def reconfigure(self, params: Dict[str, Dict[str, object]]) -> None:
        ...
        self.config = ControllerParams(
            ...
            position_weight=np.asarray(solver_cfg.get('position_weight', [10.0, 10.0, 8.0]),
                                       dtype=float),
            velocity_weight=np.asarray(solver_cfg.get('velocity_weight', [1.0, 1.0, 0.2]),
                                       dtype=float),
            quaternion_weight=np.asarray(solver_cfg.get('quaternion_weight',
                                                         [3.2, 3.2, 3.2, 3.2]),
                                         dtype=float),
            rate_weight=np.asarray(solver_cfg.get('rate_weight', [1.4, 1.4, 0.4]), dtype=float),
            control_weight=np.asarray(solver_cfg.get('control_weight', [1.75, 1.75, 1.75, 1.75]),
                                      dtype=float),
            ...
            solver_iter_max=int(solver_cfg.get('iter_max', 600)),
